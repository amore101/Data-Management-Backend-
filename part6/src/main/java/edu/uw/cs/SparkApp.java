 ...
 |-- div5tailnum: integer (nullable = true)
   */
    ...
    public static JavaRDD<Row> QB(SparkSession spark, String dataFile) {
        Dataset<Row> d = spark.read().parquet(dataFile);
        Dataset<Row> r = d.filter("cancelled != 1").groupBy(d.col("origincityname"), d.col("month")).count();
        JavaRDD<Row> rdd = r.javaRDD();
        JavaRDD<Row> r1 = rdd.map(t -> RowFactory.create(t.getString(0), t.getInt(1), t.getLong(2)));
    ...
    public static JavaRDD<Row> QC(SparkSession spark, String dataFile) {
        Dataset<Row> d = spark.read().parquet(dataFile);
        Dataset<Row> r = d.filter("depdelayminutes is not null").groupBy(d.col("origincityname")).avg("depdelayminutes");
        JavaRDD<Row> rdd = r.javaRDD();
        JavaRDD<Row> r1 = rdd.map(t -> RowFactory.create(t.getString(0), t.getDouble(1)));
